...
export default function VehiclesPage() {
  ...
  const assignedDriverByVehicle = useMemo(() => {
    ...
    for (const b of bookings) {
      if (b.status === "confirmed" && !map[b.vehicle_id])
        map[b.vehicle_id] = b.driver_name;
    }
    return map;
  }, [bookings]);
  ...
  const filtered = useMemo(() => {
    ...
    return vehicles.filter((v) => {
      const matchStatus =
        statusFilter === "all" || v.maintenance_status === statusFilter;
      const matchSearch =
        !q ||
        [v.vehicle_number, v.current_location, v.type?.name].some((s) =>
  ...
  async function toggleRetired(v: Vehicle) {
    const nextStatus =
      v.maintenance_status === "retired" ? "active" : "retired";
    try {
      const updated = await fleetApi.updateVehicle(v.id, {
        maintenance_status: nextStatus,
      });
      setVehicles((prev) =>
        prev.map((x) =>
          x.id === v.id ? { ...x, ...updated, type: x.type } : x,
        ),
      );
      toast({
        title:
          nextStatus === "retired" ? "Vehicle retired" : "Vehicle reactivated",
        variant: "success",
      });
    } catch (err) {
      toast({
        title: "Error",
        description:
          err instanceof ApiError ? err.message : "Failed to update vehicle.",
        variant: "destructive",
      });
    }
  }
  ...
  async function handleSave() {
    ...
    if (!editing && !form.vehicle_number.trim())
      return setFormError("Vehicle number is required");
    if (!editing && !form.type_id)
      return setFormError("Vehicle type is required");
    if (!form.current_location.trim())
      return setFormError("Current location is required");
    if (!cap || cap <= 0)
      return setFormError("Capacity must be a positive number");
    ...
    try {
      if (editing) {
        ...
        setVehicles((prev) =>
          prev.map((v) =>
            v.id === editing.id ? { ...v, ...updated, type: v.type } : v,
          ),
        );
    ...
    >
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center sm:justify-between">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          ...
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <Input
              placeholder="Search by number, type, location…"
              value={search}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Add your first vehicle to start dispatching."
            ...
            <TableBody>
              {filtered.map((v) => (
                ...
                >
                  ...
                  <TableCell className="text-sm">
                    {assignedDriverByVehicle[v.id] ?? "—"}
                  </TableCell>
                  <TableCell>
                    <StatusBadge status={v.maintenance_status} />
                  </TableCell>
                  {/* Edit Column */}
                  <TableCell>
                    <Button
          ...
          <div className="space-y-3 py-2">
            ...
            {!editing && (
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, vehicle_number: e.target.value }))
                  }
                />
              </div>
            )}
            {!editing && (
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {VEHICLE_TYPE_OPTIONS.filter((t) => t.id).map((t) => (
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, capacity_kg: e.target.value }))
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Bhiwandi Depot"
                value={form.current_location}
                onChange={(e) =>
                  setForm((f) => ({ ...f, current_location: e.target.value }))
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm((f) => ({ ...f, maintenance_status: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
